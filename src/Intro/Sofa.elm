module Intro.Sofa exposing (main)
...
import Intro.SofaProject exposing (..)
    ...
    let
        ...
        footer =
            wrappedRow [ width fill, spacingXY 20 10, padding 10 ]
                ...
                , el [ width fill ] <| el [ centerX ] <| text company.copyright
                ]
        blogLink=
                ...
                _ ->
                    ...
                    if String.length model.codeInput == neededLength
                        then
                            ...
                            [ header
                            , title
                            , if model.firstVisit || model.showQRcode
                                then Element.none
                                else
                                    column [centerX, width (fill |> maximum maxWidth), spacing 10 ]
                                        [ Input.text [ centerX, Font.size 15, Input.focusedOnLoad ]
                                            { onChange= ChangeCode
                                            , text= model.codeInput
                                            , placeholder= Just <|
                                                Input.placeholder [ Font.color <| gray 180, Font.size 8 ]
                                                    (text <| langGet translations.codePlacehoder)
...
translations=
    ...
    , codePlacehoder= Dict.fromList
        [ ("en", "Feature code")
        , ("de", "Ausstattungs-Code")
        ]
    ...
    , explain= Dict.fromList    -- TODO Hier werden das Produkt mit Namen und Hersteller, sowie der Model-Spender genannt 
        [ ("de", "HINWEIS: Wir selbst vertreiben keine Produkte, demonstrieren hier nur wie wir Herstellern modernste Methoden für AR bereitstellen können. Jede Variante unseres Phantasie-Produktes \"Cover\" wird über den Webshop-Button zu einer real verfügbaren Variante des englischen Anbieters für Austausch-Bezüge namens \"CoverCouch\" verlinkt! Wir stehen in keiner Geschäftsbeziehung zum Anbieter.\nDas verwendete 3D-Modell (CC0) stammt von blendswap.com")
        , ("en", "DISCLAIMER: We do not sell products, but use this to demonstrate a most modern use case of interactive AR technology for companies. All variants of our fantasy product \"Cover\" can be resolved as a real product variant by the english company \"CoverCouch\" by using the Webshop button. We are not affiliated.\nThe 3D model is from blendswap.com under a CC0 license.")
        ]
...
{-
elm make src/Intro/Sofa.elm --output=public-intro/Sofa.js
-}
