...
const fetchDocumentByFileName = (fileName) => {
  ...
  const sendFile = (str, metadata) => app.ports.infoForElm.send({tag: "GotFile", data: { fileName: metadata.fileName, id: metadata.id, content: str} })

  return readTextFile(manifestPath,  {})
     .then(value => load(value))
     .then(manifest => getMetadata(fileName, manifest))
     .then(metadata => readTextFile(filePath,  {})
     .then(str => sendFile(str, metadata)))
